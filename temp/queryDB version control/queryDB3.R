...
write_summary <- function(query){
  ...
  write.csv(summary, "Practice/sample_summary.csv")
  return()
}
...
s2Files <-  list.files(path = basePath, pattern = ".2cDNA.*.xlsx", full.names = T, recursive = TRUE)
s1Files <-  list.files(path = basePath, pattern = ".1cDNA.*.xlsx", full.names = T, recursive = TRUE)
